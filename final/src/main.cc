...
int main() {
    ...
    Orbitable Earth(Vector(-8.601208063143356e-1, -5.149780543253561e-1,
                           -7.874942568326342e-6),
                    Vector(8.657167839482775e-3, -1.475977839094277e-2,
                           8.628767650706375e-7));

    Orbitable Mars(
        Vector(-2.622004974599996e-1, 1.577561813000981, 3.925526885339471e-2),
        Vector(-1.328446880856093e-2, -1.101073656563134e-3,
               3.028803664779320e-4));

    // Asteroid
    Orbitable ToshiH(
        Vector(-7.436280142422779e-1, -2.774619548644846, 1.476905508096402e-1),
        Vector(8.942035174142673e-3, -3.065578719141641e-3,
    ...
    for (double i = init_julian; i < end_julian; i += 60) {
        ...
        for (double j = i + 60; i < end_julian; j += 60) {
            ...
            for (double k = j + 60; k < end_julian; k += 60) {
                ...
                LambertResults v1v2 =
                    lambert(new_E_pos, new_M_pos, -1, delta_t_1);
                LambertResults v3v4 =
                    lambert(new_M_pos, new_T_pos, -1, delta_t_2);
                ...
                Vector r_peri =
                    (MARSGM /
                     ((v1v2.v - new_M_veloc).dot(v1v2.v - new_M_veloc))) *
                    ((-1 / e_eps.dot(e2)) - 1) * AU / 1000;

                double rel_veloc = (v1v2.v_0 - new_E_veloc).norm();
                double dv_EG = sqrt(EARTHGM * ((2 / (1000 * earth_p / AU)) -
                                               (1 / semi_periapsis))) *
                               AU / 1000 / 86400;
